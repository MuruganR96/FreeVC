...
def process(wav_name):
    ...
    # speaker = wav_name[:4]
    dir_, wav_name = os.path.split(wav_name)
    speaker = dir_.split("/")[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sr1", type=int, default=48000, help="sampling rate")
    ...
    parser.add_argument("--out_dir1", type=str, default="./dataset/vctk-48k", help="path to target dir")
    ...
    for speaker in os.listdir(args.in_dir):
        ...
        if os.path.isdir(spk_dir):
            wavfiles = glob(os.path.join(spk_dir, "*.wav"))
            for _ in tqdm(pool.imap_unordered(process, wavfiles)):
                pass
